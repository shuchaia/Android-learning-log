...
![Java 集合框架概览](https://oss.javaguide.cn/github/javaguide/java/collection/java-collection-hierarchy.png)Java 集合框架概览
...
这里简单列举一个例子：假如我们要删除节点9 的话，需要先遍历链表找到该节点。
...
当当前数组中的元素个数与数组的容量相等，并且还要再往数组中添加元素，这时`ArrayList`就会开始扩容。

**`int newCapacity = oldCapacity + (oldCapacity >> 1)`,**所以**ArrayList 每次扩容之后容量都会变为原来的 1.5 倍左右（oldCapacity 为偶数就是 1.5 倍，否则是 1.5 倍左右）！** 奇偶不同，比如：10+10/2 = 15, 33+33/2=49。如果是奇数的话会丢掉小数。

ArrayList中的grow()方法用来扩容：
...
当需要修改（ `add`，`set`、`remove` 等操作） `CopyOnWriteArrayList` 的内容时，不会直接修改原数组，而是会先创建底层数组的副本，对副本数组进行修改，修改完之后再将修改后的数组赋值回去（对这个赋值过程加锁保证线程安全），这样就可以保证写操作不会影响读操作了。
...
注意:warning:：`CopyOnWriteArrayList` 中并没有类似于 `ArrayList` 的 `grow()` 方法扩容的操作，而是通过`Arrays.copyOf()` 复制底层数组
...
* **效率**： 因为线程安全的问题， `HashMap` 要比 `Hashtable` 效率⾼⼀点。另外， `Hashtable` 基本 被淘汰，不要在代码中使⽤它； 
...
![TreeMap 继承关系图](https://oss.javaguide.cn/github/javaguide/java/collection/treemap_hierarchy.png)TreeMap 继承关系图
...
相比于之前的版本， JDK1.8 之后在解决哈希冲突时有了较大的变化，当链表长度大于阈值（默认为 8）（将链表转换成红黑树前会判断，如果当前数组的长度小于 64，那么会选择先将数组扩容为原来的2倍，而不是转换为红黑树）时，将链表转化为红黑树，以减少搜索时间。
...
![Java7 ConcurrentHashMap 存储结构](https://oss.javaguide.cn/github/javaguide/java/collection/java7_concurrenthashmap.png)Java7 ConcurrentHashMap 存储结构
...
![Java8 ConcurrentHashMap 存储结构](https://oss.javaguide.cn/github/javaguide/java/collection/java8_concurrenthashmap.png)Java8 ConcurrentHashMap 存储结构
...
`ConcurrentHashMap`的底层跟`HashMap`一样，除了在修改数据（put）的时候会对数据加锁/CAS，不发生哈希冲突的时候用CAS直接写入，发生冲突就要获得头结点的锁，遍历更新/新增节点。

`ConcurrentHashMap` 取消了 `Segment` 分段锁，采用 `Node + CAS + synchronized` 来保证并发安全。数据结构跟 `HashMap` 1.8 的结构类似，数组+链表/红黑二叉树。Java 8 在链表长度超过一定阈值（8）时将先判断数组大小是不是 ≥64 ，如果不是，那么数组大小翻倍，否则，将链表（寻址时间复杂度为 O(N)）转换为红黑树（寻址时间复杂度为 O(log(N))）。
